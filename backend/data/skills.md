# Stack Técnico — Leandro Pablo Nuñez

## Perfil técnico
Full stack con especialización en IA generativa, RAG y sistemas agénticos.
Certificado por IBM en RAG and Agentic AI (2026).

## IA Generativa y RAG
- **RAG (Retrieval-Augmented Generation)** — Implementación end-to-end, pipeline propio sin framework
- **Embeddings vectoriales y búsqueda semántica** — Cohere (1024 dim), Gemini embeddings
- **Function calling** — Herramientas propias con hasta 8 rondas de razonamiento
- **NL2SQL** — Traducción de lenguaje natural a SQL de solo lectura, con validación por sqlglot y defensa en profundidad
- **Orquestación con LangGraph** — Máquinas de estado con reintentos y fallback entre proveedores (Groq → OpenAI)
- **Prompt engineering** y defensa contra prompt injection (keyword + semántico)
- **IA multimodal** — Formación en el certificado de IBM

## Agentes de IA
- **LangChain** — Orquestación de cadenas de razonamiento
- **LangGraph** — Grafos de estado para agentes con memoria y feedback
- **CrewAI** — Arquitecturas multi-agente colaborativas
- **AutoGen** y **BeeAI** — Frameworks de agentes
- **Model Context Protocol (MCP)** — Construcción de agentes sobre MCP
- Diseño de arquitecturas multi-agente con decisión distribuida

## Modelos y Vector DBs
- **Google Gemini** — LLM y embeddings
- **Groq (llama-3.3-70b)** — Inferencia rápida y function calling
- **Cohere** — Embeddings para búsqueda semántica
- **ChromaDB** — Vector store
- **pgvector** — Vectores dentro de PostgreSQL

## Frontend
- **React 19** — Intermedio
- **TypeScript** — Intermedio
- **JavaScript (ES6+)** — Intermedio
- **TanStack Router / TanStack Query** — Routing y data fetching type-safe
- **Zustand** — Estado global
- **Tailwind CSS** y **Shadcn UI**
- **React Hook Form + Zod** — Formularios y validación
- **HTML5 / CSS3** — Avanzado
- **Vite**

## Backend
- **Node.js / Express** — Intermedio
- **Python / FastAPI** — Intermedio
- **SQLAlchemy 2.0 + Alembic** — ORM y migraciones de esquema versionadas
- **Arquitectura multi-tenant** — Aislamiento por Row-Level Security, monolito modular package-by-feature, capa service separada del router
- **REST APIs** — Diseño e implementación
- **JWT** — Access + refresh token, cookies HttpOnly, RBAC
- **SSE streaming** — Respuestas en streaming
- **C#** — Básico

## Bases de Datos
- **PostgreSQL** — Intermedio (Prisma, SQLAlchemy, pgvector, Alembic, full-text search)
- **Row-Level Security (RLS)** — Aislamiento multi-tenant a nivel de base, con roles sin BYPASSRLS
- **Supabase** — Postgres gestionado + Auth
- **MySQL** — Intermedio
- **Prisma** — ORM type-safe
- Modelado relacional

## Testing
- **Vitest** — Unit y component testing
- **React Testing Library** — Testing de componentes por comportamiento
- **Supertest** — Integration testing de APIs contra PostgreSQL real
- Test suite completa implementada en PremiumTech, corriendo en CI

## DevOps / Herramientas
- **Git / GitHub** — Ramas, PRs, conventional commits
- **GitHub Actions** — CI/CD con deploy continuo
- **Docker** — Containerización y deploy (usado en el chatbot RAG)
- **Vercel**, **Render**, **Hugging Face Spaces**, **Neon**, **Cloudinary**
- **Linux** — Intermedio
- **Bash scripting** — Básico-Intermedio
- **Neovim** — Editor principal
- **Claude Code** — Desarrollo asistido por IA
